...
# calculate lambda for age group i using both naive and repeat infections: *Esha
      ...
      infection <- beta0*(1+ beta1*cos(2*pi*(time)/12+phi)) * sum(data.contact.monthly[i, ] * (I_naive+(sigma*I_subsequent))/N) 
      # naive
      
      # dSi0/dt
      ODES[i] = mu + (tau_previous * S0[i-1]) - (infection * sigma + tau_current) * S0[i]
      
      # dEi0/dt
      ODES[num_grp+i] = (tau_previous * E0[i-1]) + (infection * sigma * S0[i]) - (delta + tau_current) * E0[i]
      ...
      # dSi1/dt 
      ODES[num_grp*4+i] = (tau_previous * S1[i-1]) - (infection * sigma + tau_current) - nu(R1[i] + R0[i])
